...
from deep_vision_tool.image_preprocessing.create_data_object import imageObjectCreation
from deep_vision_tool.image_object.annotation import Annotation
import pytest
def test_obj(tmp_path):
    objimage_yolo = imageObjectCreation("data_folder/yolo/","yolo", tmp_path, "data_folder/training_images/","logs")
    objimage_coco = imageObjectCreation("data_folder/coco/","coco", tmp_path, "data_folder/training_images/","logs")
    ...
    for iminfo in objimage_coco.coco_image_obj:
        assert isinstance(iminfo.filename, str)
        assert isinstance(iminfo.image_path, str)
        assert isinstance(iminfo.annotations[0], Annotation)
        assert isinstance(iminfo.im_width, int)
